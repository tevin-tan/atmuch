...
class RunTestSuite(object):
	...
	def Add_test_case_run_02(self):
		...
		# 登录测试用例添加到测试套
		test_login = [
			zendaologin.TestLogin('test_login_01_success'),
			zendaologin.TestLogin('test_login_02_error_name'),
			zendaologin.TestLogin('test_login_03_error_password'),
			zendaologin.TestLogin('test_login_04_name_empty'),
			zendaologin.TestLogin('test_login_05_password_empty'),
			zendaologin.TestLogin('test_login_06_many_times')
		]
		suite.addTests(test_login)

		test_create_program = [create_programe.Create_program('test_create_program_01')]
	...
	def Add_test_case_run_03(self):
		...
		'''方法一'''
		# 构造测试套件，添加测试用例
		suite.addTest(zendaologin.TestLogin('test_login_01_success'))
		suite.addTest(zendaologin.TestLogin('test_login_02_error_name'))
		suite.addTest(zendaologin.TestLogin('test_login_03_error_password'))
		suite.addTest(zendaologin.TestLogin('test_login_04_name_empty'))
		suite.addTest(zendaologin.TestLogin('test_login_05_password_empty'))
		suite.addTest(zendaologin.TestLogin('test_login_06_many_times'))
		return suite
