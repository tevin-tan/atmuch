...
class RunTestSuite(object):
    ...
    def Add_test_case_run(self):
        ...
        A = cases.test_create_product
        A_case = [A.create_product, A.product_document, A.product_plan, A.product_publish]
        for i in A_case:
            cs = unittest.TestLoader().loadTestsFromTestCase(i)
            self.t_s.append(cs)

        self.t_s.append(suite1)
        self.t_s.append(suite2)
        suite = unittest.TestSuite(self.t_s)
